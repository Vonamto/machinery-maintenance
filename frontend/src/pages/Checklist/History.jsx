...
export default function ChecklistHistory() {
  ...
  const loadChecklists = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Assign original Google Sheet row index
        ...
        if (user?.role === "Driver") {
          ...
          const allowedPlates = driverEquipment.map(
            eq => eq["Plate Number"]
          );

          const filtered = sorted.filter(record =>
            allowedPlates.includes(record["Plate Number"])
          );
  ...
  const filteredChecklists = checklists.filter(record => {
    const matchesDriver =
      !filters.driver ||
      record["Full Name"] === filters.driver;

    const matchesPlate =
      !filters["Plate Number"] ||
      record["Plate Number"] === filters["Plate Number"];

    const matchesModel =
      !filters["Model / Type"] ||
      record["Model / Type"] === filters["Model / Type"];
  ...
  const handleDelete = async rowIndexInPage => {
    ...
    if (!rowToDelete?.__row_index) {
      alert(
        t("checklist.history.deleteError") ||
          "Error: Unable to identify the record to delete."
      );
      return;
    }
    ...
    try {
      ...
      if (result.status === "success") {
        setChecklists(prev =>
          prev.filter(
            r =>
              r.__row_index !== rowToDelete.__row_index
          )
        );

        alert(
          t("checklist.history.deleteSuccess") ||
  ...
  const calculateSummary = checklistData => {
    ...
    Object.values(checklistData).forEach(item => {
      if (
        item.status &&
        summary.hasOwnProperty(item.status)
      ) {
        summary[item.status]++;
      }
    });
  ...
  const calculateSectionSummary = (
    ...
    section.items.forEach(item => {
      ...
      if (
        itemData?.status &&
        summary.hasOwnProperty(itemData.status)
      ) {
  ...
  const plateNumbers = [
    ...new Set(
      equipment.map(eq => eq["Plate Number"])
    )
  ];

  const models = [
    ...new Set(
      equipment.map(eq => eq["Model / Type"])
    )
  ];
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">
                    {t(
                      "checklist.history.filterDriver"
                    ) || "Filter by Driver"}
                  </option>
            ...
            <div className="flex justify-between mt-4">
              ...
              {canDelete && (
                ...
                >
                  ...
                  {deleteMode
                    ? t("common.cancel")
                    : t(
                        "equipment.manage.actions.delete"
                      )}
                </button>
              )}
              ...
              >
                ...
                {t(
                  "maintenance.history.filters.reset"
                )}
              </button>
        ...
        {paginatedChecklists.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {t(
                "checklist.history.noResultsDescription"
              ) ||
          ...
          <div className="space-y-4">
            ...
            {paginatedChecklists.map(
              (record, index) => {
                ...
                try {
                  checklistData = JSON.parse(
                    record["Checklist Data"] || "{}"
                  );
                } catch (e) {
                  console.error(
                    "Failed to parse checklist data:",
                    e
                  );
                }

                const summary =
                  calculateSummary(checklistData);
                                            ...
                                            >
                                              ...
                                              <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="flex items-center gap-3">
                                                  ...
                                                  <span className="text-sm text-gray-400">
                                                    {t(
                                                      `checklist.statusLabels.${itemData.status}`
                                                    ) ||
                                                      itemData.status}
                                                  </span>
