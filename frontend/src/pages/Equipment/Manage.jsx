...
export default function EquipmentManage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-red-400 mb-2">{t("equipment.manage.accessDenied.title")}</h1>
          <p className="text-gray-300">{t("equipment.manage.accessDenied.message")}</p>
          <button
            onClick={() => navigate(-1)}
            className="mt-6 px-6 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg transition"
          >
            {t("equipment.manage.accessDenied.goBack")}
          </button>
        </div>
      </div>
    );
  ...
  const handleAddEquipment = async (e) => {
    ...
    if (!newEquipment["Model / Type"] || !newEquipment["Plate Number"]) {
      alert(t("equipment.manage.alerts.fillRequiredFields"));
      return;
    }
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        alert(t("equipment.manage.alerts.addError", { message: data.message || t("common.error") }));
      }
    } catch (err) {
      console.error("Add equipment error:", err);
      alert(t("equipment.manage.alerts.networkError"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteEquipment = async (rowIndex, plateNumber) => {
    const confirmed = window.confirm(
      t("equipment.manage.alerts.deleteConfirm", { plateNumber })
    );
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        alert(t("equipment.manage.alerts.deleteError", { message: data.message || t("common.error") }));
      }
    } catch (err) {
      console.error("Delete equipment error:", err);
      alert(t("equipment.manage.alerts.deleteNetworkError"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    if (!data["Model / Type"] || !data["Plate Number"]) {
      alert(t("equipment.manage.alerts.fillRequiredFieldsEdit"));
      return;
    }
    ...
    try {
      ...
      if (result.status === "success") {
        alert(t("equipment.manage.alerts.updateSuccess"));
        setEditingRow(null);
        loadEquipment();
      } else {
        alert(t("equipment.manage.alerts.updateError", { message: result.message || t("equipment.manage.alerts.updateNetworkError") }));
      }
    } catch (err) {
      console.error("Edit equipment error:", err);
      alert(t("equipment.manage.alerts.updateNetworkError"));
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg">{t("equipment.manage.loading")}</p>
        </div>
      </div>
    );
  }
        ...
        {showAddForm && (
          <div className="mb-8 bg-gray-800/40 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 shadow-lg">
            ...
            <form onSubmit={handleAddEquipment} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder={t("equipment.manage.addForm.placeholder.modelType")}
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-3 rounded-xl bg-gray-900/70 border border-gray-700 text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition-all"
                    placeholder={t("equipment.manage.addForm.placeholder.plateNumber")}
                    required
                  />
                </div>
              ...
              >
                {saving ? t("equipment.manage.addForm.submitting") : t("equipment.manage.addForm.submit")}
              </button>
            </form>
          </div>
        )}
          ...
          <div className="mb-8 bg-gray-800/40 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 shadow-lg">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  {saving ? t("equipment.manage.editForm.saveSubmitting") : t("equipment.manage.editForm.saveChanges")}
                </button>
                <button
                  type="button"
                  onClick={cancelEdit}
            ...
            <tbody>
              {rows.map((r, i) => (
                ...
                >
                  ...
                  <td className="p-4 text-sm">{t(`status.${r["Status"]}`) || r["Status"]}</td>
